...
export function buildAiMessages(input: PromptInput) {
  ...
  const userPrompt = [
    profileLine,
    "请用盲派技巧逐步分析八字，详细测算一生综合运势，事业、财运、婚姻、健康等各方面内容，均需具体细致、全面解读，你可以自我扩宽分析面，但是要确保详细准确。分析中请减少对具体八字术语、逐柱拆解和命盘细节的篇幅，重点解释重要结论、运势趋势和现实层面的影响，使用通俗易懂且令人信服的文字进行描述。如果涉及关键结论、重要提醒或核心判断，可以使用 Markdown 的 **加粗** 进行强调。",
  ].join("\n");
  ...
  return [
    {
      role: "system",
      content:
        "你是中文八字分析助手。请直接输出详细正文，不要输出 JSON，不要输出代码块。为了便于前端切分展示，请使用 Markdown 一级标题格式分段，也就是每个大段都以 `# 标题` 开头。除了使用一级标题分段之外，不要人为压缩、限制或模板化你的分析结构。如果涉及重要结论、关键提醒或需要突出显示的判断，可以使用 Markdown 的 **加粗** 进行强调。",
    },
    {
      role: "user",
      content: userPrompt,
