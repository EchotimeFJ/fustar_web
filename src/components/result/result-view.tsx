    ...
    <div className="space-y-8">
      <section className="mystic-panel mystic-border rounded-[36px] p-6 md:p-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="text-sm font-medium tracking-[0.24em] text-[#cfb67c] uppercase">Mystic Reading</div>
            <h1 className="mt-2 text-3xl font-semibold text-white md:text-4xl">{reading.profile.eightChar}</h1>
            <p className="mt-3 text-sm leading-7 text-[#d2c7b0]">
              {reading.profile.genderLabel}命 · {reading.profile.birthplace} · {reading.profile.solarText} · {reading.profile.lunarText}
            </p>
          </div>
          <div className="rounded-2xl border border-white/8 bg-white/5 px-4 py-3 text-xs leading-6 text-[#bcae90]">
      ...
      <div className="grid gap-6 xl:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="xl:sticky xl:top-24 xl:self-start">
          <div className="mystic-panel mystic-border rounded-[32px] p-5">
            <div className="text-xs font-semibold tracking-[0.22em] text-[#cfb67c] uppercase">Reading Map</div>
            <div className="mt-5 space-y-2">
              {sectionLinks.map((section, index) => (
                ...
                >
                  <span className="mr-2 text-[#9f8c67]">{String(index + 1).padStart(2, "0")}</span>
                  {section.title}
                </a>
              ))}
            </div>

            <div className="mt-6 space-y-2 text-sm text-[#bcae90]">
        ...
        <section className="grid gap-6">
          {reading.sections.map((section) => (
            <section id={section.id} key={section.id} className="mystic-panel mystic-border rounded-[32px] p-6 md:p-8">
              ...
              <div className="mt-5 space-y-4 whitespace-pre-wrap text-[15px] leading-8 text-[#d2c7b0]">
                {section.content}
              </div>
            </section>
          ))}
      ...
      <section className="mystic-panel mystic-border rounded-[32px] p-6">
        ...
        <div className="mt-4 space-y-3 text-sm leading-7 text-[#bcae90]">
